...
Water::Water(int x,int y, int w, int h,b2World* world,SDL_Texture* texture){
		m_position.x = x;
		m_position.y = y;
		m_position.w = w;
		m_position.h = h; 
...
void Water::Render(SDL_Renderer* render)
{
	b2Vec2 position;
	m_position.x = body->GetPosition().x;
	m_position.y = body->GetPosition().y;
	SDL_Rect posImg = {m_position.x - (m_position.w/2), m_position.y - (m_position.h/2),m_position.w,m_position.h};

	SDL_RenderCopy(render,m_texture,NULL,&posImg);

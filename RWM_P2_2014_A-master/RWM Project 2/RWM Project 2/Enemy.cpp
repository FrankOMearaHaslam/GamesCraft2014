...
Enemy::Enemy(b2World* world, int x, int y) : m_angle(0.0f){
	...
	dynamicBody = BodyFactory::createBody(world, pos, size, 1, 1, true, false, myColIdent,	Filtering::ENEMY, Filtering::PLAYER | Filtering::BLOCK | Filtering::ELEVATOR);
...
void Enemy::Draw(SDL_Renderer* gRenderer, b2Vec2 offset){
	...
	SDL_RenderCopy( gRenderer, TextureManager::getManager()->enemyTexture, NULL, &stretchRect);
